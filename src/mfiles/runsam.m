...
function [P,Pci,Pcc] = runsam(A,method,ci_flag,alpha,oopt_flag,cc_flag,header,H,randomseed,nperms)

%% Run R
samdata = A(2:end,:);
samlabel = A(1,:);
save([pwd '/inputs/samdata.txt'],'samdata','-ascii');
save([pwd '/inputs/samlabel.txt'],'samlabel','-ascii');
nperms = [nperms randomseed];
save([pwd '/inputs/samvar.txt'],'nperms','-ascii');
system(['/tools64/R-2.11.1/bin/R CMD BATCH --no-restore --no-save ' pwd '/mfiles/runsam.r ' pwd '/logs/samR.log']);
if ~exist([pwd '/inputs/orgstat.txt'],'file')|~exist([pwd '/inputs/permstat.txt'],'file');
    error('SAM in R failed, see samR.log for details');
end
orgstat = load([pwd '/inputs/orgstat.txt'],'-ascii');
permstat = load([pwd '/inputs/permstat.txt'],'-ascii');
orgstat = abs(orgstat);
C = length(orgstat);
permstat = abs(permstat(:));
R = length(permstat);
