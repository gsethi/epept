...
#my ($matrixFile, $host, $outputPath, $params) = @ARGV;
#permutation parameters  
my $matrixFile = "C:\\EPEPT\\data\\yeastdata.txt";
my $gseaFile = "C:\\EPEPT\\data\\geneset.gmt";

my $host = "http://informatics.systemsbiology.net";
my $outputPath = "C:\\EPEPT\\out";
my $params = "{mode:'GSEA',method:'PWM',ci_chk:'true',ci:'49',oopt_chk:'false',cc_chk:'false',mail_address:'jlin212\@gmail.com'}";
my $json = new JSON;
my $obj;
my $url;
...
sub epept_makerequest() {
   ...
   if($url) {
      my $response = $ua->post($url, Content_Type => 'form-data', Content => [filename => [$matrixFile], gsfile => [$gseaFile]]);
